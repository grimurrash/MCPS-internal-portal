...
export default {
  ...
  setup() {
    ...
    watch([currentPage, perPage, searchQuery, departmentFilter, employeeFilter, selectDate], () => {
      refetchData()
    })

    const fetchVisitEvents = (ctx, callback) => {
      axios.get('/management/visit-events/absence-work', {
        params: {
          q: searchQuery.value,
    ...
    const exportVisitEvents = () => {
      axios.post('/management/visit-events/absence-work/export', {
        q: searchQuery.value,
        perPage: perPage.value,
        page: currentPage.value,
        sortBy: sortBy.value,
        ...
        .then(response => {
          FileDownload(response.data, 'Экспорт событий (Отсутствие на работе).xlsx')
        })
        .catch(() => {
          toast({
            component: ToastificationContent,
