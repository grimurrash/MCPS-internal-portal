<template>
  <b-sidebar
    id="edit-template-sidebar"
    :visible="isEditTemplateSidebarActive"
    bg-variant="white"
    ...
    right
    @hidden="resetForm"
    @change="(val) => $emit('update:is-edit-template-sidebar-active', val)"
  >
    <template #default="{ hide }">
      <!-- Header -->
      <div class="d-flex justify-content-between align-items-center content-sidebar-header px-2 py-1">
        <h5 class="mb-0">
          Измененить шаблон документа
        </h5>
        ...
        >
          ...
          >
            <b-form-file
              ...
              accept=".docx, .doc."
              placeholder="Выберите новый файл или перетащите его сюда..."
...
export default {
  ...
  model: {
    prop: 'isEditTemplateSidebarActive',
    event: 'update:is-edit-template-sidebar-active',
  },
  props: {
  ...
  watch: {
    templateId(editTemplate) {
      if (editTemplate.id === 0) return
      this.template = {
        id: editTemplate.id,
        name: editTemplate.name,
        description: editTemplate.description,
        file: null,
        documentType_id: editTemplate.type_id,
        isDouble: editTemplate.isDouble ? 1 : 0,
      }
    },
  },
  methods: {
    ...
    onSubmit() {
      this.$refs.refFormObserver.validate()
        .then(success => {
          ...
          if (this.isChangeFile) formData.append('file', this.template.file)

          axios.post(`/documents/templates/${this.template.id}`, formData, {
            headers: {
