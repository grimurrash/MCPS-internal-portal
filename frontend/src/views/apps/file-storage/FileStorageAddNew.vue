...
export default {
  ...
  methods: {
    handleFileUpload() {
      this.item.file = this.$refs.fileInput.files[0]
    },
    onSubmit() {
      this.$refs.refFormObserver.validate()
        .then(success => {
          ...
          axios.post('/file-storage', formData, {
            headers: {
  ...
  setup() {
    ...
    const item = ref(JSON.parse(JSON.stringify(blankUserData)))
    const resetuserData = () => {
      item.value = JSON.parse(JSON.stringify(blankUserData))
    }
    const file = ref(null)
    const {
      refFormObserver,
